...
locals {
  ...
  tags = merge(include.root.locals.root_tags, local.local_tags)
}

dependency "vpc" {
  config_path                             = "${get_parent_terragrunt_dir()}/base/vpc"
...
terraform {
  source = "${get_parent_terragrunt_dir("root")}/..//modules/eks"
}